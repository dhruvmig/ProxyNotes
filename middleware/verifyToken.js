...
async function verifyToken(req, res, next) {
    try {
        if (!req.headers.authorization) {
             res.status(401).send("unauthorized access in req.headers");
        }
        console.log(req.headers);
        let token = req.headers.authorization.split(' ')[1];
        if (token === null) {
             res.status(402).send("unauthorized access in token nlll");
        }
        let payload = helper.verifyToken(token);
        if (!payload) {
             res.status(403).send("unauthorized access in payload");
        }
